...
from src.decoding import beam_search, ensemble_beam_search, biGrid_beam_search, fixwords_beam_search
...
def sample_ordered_constrains(batch_result, batch_ref, patience):
    batch_size = len(batch_result)
    constrains = [[] for i in range(batch_size)]
    positions = [[] for i in range(batch_size)]

    for idx in range(batch_size):
        sent = batch_result[idx]
        ref = batch_ref[idx]
        flag = False
        for j in range(min(len(sent), len(ref))):
            if sent[j] != ref[j]:
                constrains[idx].append(ref[j])
                positions[idx].append(j)
                flag = True
                break
        if flag == False:
            constrains[idx].append(ref[0])
            positions[idx].append(0)

    return constrains, positions

def sample_constrains(batch_result, batch_ref, patience):
    batch_size = len(batch_result)
    constrains = [[] for i in range(batch_size)]
    positions = [[] for i in range(batch_size)]
    ...
    for idx in range(len(batch_result)):
        remaining_num = patience
        sent = batch_result[idx]
        ref = batch_ref[idx]
        for i in range(n_gram, 0, -1):
            for pos in range(len(sent) - i + 1):
                gram = sent[pos:pos + i]
                for j in range(len(ref) - i + 1):
                    if gram == ref[j:j + i]:
                        if j > 0:
                            remaining_num -= 1
                            constrains[idx].append(ref[j - 1])
                            positions[idx].append(pos - 1)
                        if remaining_num <= 0:
                            break
                        if j + i < len(ref) - 1:
                            remaining_num -= 1
                            constrains[idx].append(ref[j + i])
                            positions[idx].append(pos + i)
                        if remaining_num <= 0:
                            break
                if remaining_num <= 0:
                    break
        ...
        if remaining_num > 0:
            for i in range(remaining_num):
                constrains[idx].append(ref[i % len(ref)])
                positions[idx].append(i % len(ref))

    return constrains, positions

#cpmpute bleu score(simple version)
N_gram = 4

def countTimes(sent, n_gram_words):
    js = 0
    n_gram = len(n_gram_words)
    num_ref = len(sent)
    for i in range(num_ref - n_gram + 1):
        flag = True
        for j in range(n_gram):
            if sent[i + j] != n_gram_words[j]:
                flag = False
        if flag:
            js += 1
    return js

def bleuScore(src, ref):
    src_words = src
    ref_words = ref
    c = len(src_words)
    r = len(ref_words)
    ...
    Bleu = [0.0 for i in range(N_gram)]
    for i in range(N_gram):
        p = 0.0
        for j in range(c - i):
            n_gram_words = src_words[j:j + i + 1]
            if countTimes(src_words[:j], n_gram_words) == 0:
                t_src = countTimes(src_words, n_gram_words)
                t_ref = countTimes(ref_words, n_gram_words)
                t = min(t_src, t_ref)
                p += float(t) / (r - i)
        Bleu[i] = p
    BLEU = 0.0
...
def interactive_FBS(FLAGS):
    ...
    for batch in valid_iter:
        ...
        with torch.no_grad():
            word_ids = beam_search(nmt_model=fw_nmt_model, beam_size=FLAGS.beam_size,
                                   max_steps=FLAGS.max_steps, src_seqs=x, alpha=FLAGS.alpha)
        ...
        constrains = [[[] for ii in range(patience)] for jj in range(batch_size_t)]
        positions = [[[] for ii in range(patience)] for jj in range(batch_size_t)]
        for idx in range(FLAGS.imt_step):
            ...
            if FLAGS.bidirection:
                bidirection = True
            with torch.no_grad():
                constrained_word_ids, positions = fixwords_beam_search(fw_nmt_model=fw_nmt_model, bw_nmt_model=bw_nmt_model,
                                                      beam_size=FLAGS.beam_size, max_steps=FLAGS.max_steps,
                                                      src_seqs=x, alpha=FLAGS.alpha, constrains=constrains, positions=positions,
                                                      last_sentences=last_sents, imt_step=idx+1, bidirection=bidirection)
            ...
            if FLAGS.online_learning and idx == FLAGS.imt_step - 1:
                ...
                for sent in last_sents:
                    sent = [BOS] + sent
                    seqs_y.append(sent)
                compute_forward(fw_nmt_model, critic, x, torch.Tensor(seqs_y).long().cuda())
                seqs_y = [sent[::-1] for sent in seqs_y]
                for ii in range(len(seqs_y)):
                    seqs_y[ii][0] = BOS
                    seqs_y[ii][-1] = EOS
                compute_forward(bw_nmt_model, critic, x, torch.Tensor(seqs_y).long().cuda())
    ...
    for idx in range(FLAGS.imt_step):
        imt_constrains[idx] = [' '.join(imt_constrains[idx][ii]) + '\n' for ii in origin_order]
